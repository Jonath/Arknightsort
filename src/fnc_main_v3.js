...
function startup() {
   ...
   new_cell.appendChild(new_span);
	...
	//stage table
   var tbl_Stage = gID('stageTable');
   
      var tbl_body_Stage = cE('tbody');
   tbl_Stage.appendChild(tbl_body_Stage);

   // ã‚¿ã‚¤ãƒˆãƒ«ã‹ã‚‰é¸æŠžç”¨ãƒã‚§ãƒƒã‚¯ãƒœãƒƒã‚¯ã‚¹ã«å¤‰æ›
   for (i=0; i<ary_StageData.length; i++) {
      // Row[i]
      if ((i % int_Colspan) == 0) {
         var new_row = tbl_body_Stage.insertRow(tbl_body_Stage.rows.length);
         new_row.id = 'optSelRow' + i;
      }
      // Col[0]
      var new_cell = new_row.insertCell(new_row.childNodes.length);
      ...
      new_CheckBox.setAttribute('type', 'checkbox', 0);
      new_CheckBox.setAttribute('checked', 'true', 0);
      new_CheckBox.value = ary_StageData[i];
      new_CheckBox.title = ary_StageData[i];
      new_CheckBox.id = 'optStage' + i;
      new_cell.appendChild(new_CheckBox);

      var new_span = cE('span');
      new_span.appendChild(cT(ary_StageData[i]));
      new_span.title = ary_StageData[i];
      new_span.id = i;
      sC(new_span, 'cbox');
      new_span.onclick = function() {chgFlag(this.id);}
      new_cell.appendChild(new_span);
   ...
   var tbl_foot_Stage = cE('tfoot');
   tbl_Stage.appendChild(tbl_foot_Stage);

   // Row[0]
   var new_row = tbl_foot_Stage.insertRow(tbl_foot_Stage.rows.length);
   sC(new_row, "opt_foot");
   ...
   gID('optStage').disabled = false;
   gID('optStage').checked = false;
   gID('optSelect_all').disabled = false;
      for (i=0; i<ary_StageData.length; i++) {
         gID('optStage' + i).disabled = true;
      }
...
function SwitchDisabled() {
      for (i=0; i<ary_StageData.length; i++) {
         gID('optStage' + i).disabled = !gID('optStage').checked;
      }
}
...
function init(){
   int_Total = 0;
   int_RecordID = 0;
   var stageFilter = gID('optStage').checked;
   ...
   for (i=0; i<ary_CharacterData.length; i++) {
      for (j=0; j<ary_TitleData.length; j++) {
         if (gID('optSelect' + j).checked && (ary_CharacterData[i][2][j] == 1)) {
			for(k=0; k<ary_StageData.length; k++) {
				if(gID('optStage' + k).checked && (ary_CharacterData[i][4][k] == 1)) {
            		ary_TempData[int_Total] = ary_CharacterData[i];
            		int_Total++;
					breaku = true;
            		break;
...
function fnc_Save() {
   ...
   $.jStorage.set("TohoCharSorter_ary_EqualData", ary_EqualData, null)
   $.jStorage.set("TohoCharSorter_ary_ParentData", ary_ParentData, null)
   $.jStorage.set("TohoCharSorter_ary_RecordData", ary_RecordData, null)
   $.jStorage.set("TohoCharSorter_ary_SortData", ary_SortData, null)
   $.jStorage.set("TohoCharSorter_ary_TempData", ary_TempData, null)
   
   $.jStorage.set("TohoCharSorter_back_ary_EqualData", back_ary_EqualData, null)
   $.jStorage.set("TohoCharSorter_back_ary_ParentData", back_ary_ParentData, null)
   $.jStorage.set("TohoCharSorter_back_ary_RecordData", back_ary_RecordData, null)
   $.jStorage.set("TohoCharSorter_back_ary_SortData", back_ary_SortData, null)
   $.jStorage.set("TohoCharSorter_back_int_Completed", back_int_Completed, null)
   
   $.jStorage.set("TohoCharSorter_back_int_LeftID", back_int_LeftID, null)
   $.jStorage.set("TohoCharSorter_back_int_LeftList", back_int_LeftList, null)
   $.jStorage.set("TohoCharSorter_back_int_RecordID", back_int_RecordID, null)
   $.jStorage.set("TohoCharSorter_back_int_RightID", back_int_RightID, null)
   $.jStorage.set("TohoCharSorter_back_int_RightList", back_int_RightList, null)
   
   $.jStorage.set("TohoCharSorter_back_int_Total", back_int_Total, null)
   $.jStorage.set("TohoCharSorter_int_Completed", int_Completed, null)
   $.jStorage.set("TohoCharSorter_int_Count", int_Count, null)
   $.jStorage.set("TohoCharSorter_int_LeftID", int_LeftID, null)
   $.jStorage.set("TohoCharSorter_int_LeftList", int_LeftList, null)
   
   $.jStorage.set("TohoCharSorter_int_RecordID", int_RecordID, null)
   $.jStorage.set("TohoCharSorter_int_RightID", int_RightID, null)
   $.jStorage.set("TohoCharSorter_int_RightList", int_RightList, null)
   $.jStorage.set("TohoCharSorter_int_Status", int_Status, null)
   $.jStorage.set("TohoCharSorter_int_Total", int_Total, null)
}

function fnc_Load() {	
	if($.jStorage.get("TohoCharSorter_int_Total", "swap") != "swap") {
		...
		ary_EqualData = $.jStorage.get("TohoCharSorter_ary_EqualData");
		ary_ParentData = $.jStorage.get("TohoCharSorter_ary_ParentData");
		ary_RecordData = $.jStorage.get("TohoCharSorter_ary_RecordData");
		ary_SortData = $.jStorage.get("TohoCharSorter_ary_SortData");
		ary_TempData = $.jStorage.get("TohoCharSorter_ary_TempData");
		 
		back_ary_EqualData = $.jStorage.get("TohoCharSorter_back_ary_EqualData");
		back_ary_ParentData = $.jStorage.get("TohoCharSorter_back_ary_ParentData");
		back_ary_RecordData = $.jStorage.get("TohoCharSorter_back_ary_RecordData");
		back_ary_SortData = $.jStorage.get("TohoCharSorter_back_ary_SortData");
		back_int_Completed = $.jStorage.get("TohoCharSorter_back_int_Completed");
		 
		back_int_LeftID = $.jStorage.get("TohoCharSorter_back_int_LeftID");
		back_int_LeftList = $.jStorage.get("TohoCharSorter_back_int_LeftList");
		back_int_RecordID = $.jStorage.get("TohoCharSorter_back_int_RecordID");
		back_int_RightID = $.jStorage.get("TohoCharSorter_back_int_RightID");
		back_int_RightList = $.jStorage.get("TohoCharSorter_back_int_RightList");
		 
		back_int_Total = $.jStorage.get("TohoCharSorter_back_int_Total");
		int_Completed = $.jStorage.get("TohoCharSorter_int_Completed");
		int_Count = $.jStorage.get("TohoCharSorter_int_Count");
		int_LeftID = $.jStorage.get("TohoCharSorter_int_LeftID");
		int_LeftList = $.jStorage.get("TohoCharSorter_int_LeftList");
		 
		int_RecordID = $.jStorage.get("TohoCharSorter_int_RecordID");
		int_RightID = $.jStorage.get("TohoCharSorter_int_RightID");
		int_RightList = $.jStorage.get("TohoCharSorter_int_RightList");
		int_Status = $.jStorage.get("TohoCharSorter_int_Status");
		int_Total = $.jStorage.get("TohoCharSorter_int_Total");
